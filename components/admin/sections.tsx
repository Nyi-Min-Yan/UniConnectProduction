...
import { useConversations, useEvents, useEventRegistrations } from '@/lib/hooks';
...
export function Dashboard() {
  ...
  const { registrations } = useEventRegistrations(eventIds, me);
...
export function TimetableSection() {
  ...
  const exportDownloadUrl = activeTerm
    ? `/api/export/timetable?source=published&termId=${encodeURIComponent(activeTerm.termId)}`
    : '';
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18, flexWrap: 'wrap' }}>
        ...
        <button
          onClick={() => activeTerm && setShowExport(true)}
          disabled={!activeTerm}
          style={{ background: 'linear-gradient(var(--primary), var(--primary-dark))', color: '#fff', borderRadius: 'var(--radius-sm)', padding: '8px 16px', fontSize: 13, fontWeight: 600, border: 'none', cursor: activeTerm ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', gap: 6, opacity: activeTerm ? 1 : 0.5 }}>
          <Download size={14} /> Export
        </button>
      </div>
      {(terms.loading || schedules.loading) && !terms.data && (
        <div className="bg-base-100 backdrop-blur-xl" style={{ borderRadius: 'var(--radius-lg)', border: '1px solid var(--surface-border)', boxShadow: 'var(--shadow-sm)', overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 18px', borderBottom: '1px solid var(--surface)' }}>
            <div className="skeleton h-4 w-44" style={{ borderRadius: 6 }} />
            <div className="skeleton h-3 w-24" style={{ borderRadius: 6 }} />
          </div>
          <div style={{ padding: 1, background: 'var(--secondary)' }}>
            <div style={{ overflowX: 'auto' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '120px repeat(7, 1fr)', gap: 1, minWidth: 780 }}>
                {Array.from({ length: 5 * 8 }).map((_, i) => (
                  <div key={i} style={{ height: 56, background: 'var(--surface-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div className="skeleton h-3" style={{ width: i % 8 === 0 ? 70 : '70%', borderRadius: 6 }} />
                  </div>
                ))}
              </div>
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
          {dayByGroup.map(({ group, days }) => (
            <div key={`${group.semesterNo}|${group.section}`} className="bg-base-100 backdrop-blur-xl" style={{ borderRadius: 'var(--radius-lg)', border: '1px solid var(--surface-border)', boxShadow: 'var(--shadow-sm)', overflow: 'hidden' }}>
              ...
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', minWidth: 780, tableLayout: 'fixed', borderCollapse: 'separate', borderSpacing: 1, background: 'var(--secondary)' }}>
                  <thead>
                    <tr>
                      <th style={{ background: 'linear-gradient(135deg, var(--primary), var(--primary-dark))', padding: '12px 8px', fontSize: 12, textAlign: 'center', fontWeight: 700, color: '#fff', position: 'sticky', left: 0, zIndex: 3, width: 120 }}>Day</th>
                      {[1, 2, 3, 4, 5, 6, 7].map((r) => {
                        const periodNo = r <= 3 ? r : r - 1;
                        const timeLabel = r === 4 ? timeGrid.lunchLabel : (timeGrid.periodLabels[periodNo - 1] ?? '');
                        return (
                          <th key={r} style={{ background: 'linear-gradient(135deg, var(--primary), var(--primary-dark))', padding: '12px 8px', fontSize: 11, textAlign: 'center', fontWeight: 700, color: '#fff', whiteSpace: 'nowrap' }}>
                            {r === 4 ? 'Lunch' : `P${periodNo}`}
                            <div style={{ fontSize: 9.5, fontWeight: 500, opacity: 0.85, marginTop: 1 }}>{timeLabel}</div>
                          </th>
                        );
                      })}
                    </tr>
                  ...
                  <tbody>
                    {['Mon', 'Tue', 'Wed', 'Thu', 'Fri'].map((name, di) => {
                      const dayCells = days[di];
                      return (
                        <tr key={name}>
                          <th scope="row" style={{ position: 'sticky', left: 0, zIndex: 2, background: 'var(--secondary-lighter)', padding: '12px 8px', fontSize: 12, textAlign: 'center', fontWeight: 800, color: 'var(--accent)', height: 64, boxShadow: '1px 0 0 var(--surface-border)' }}>{name}</th>
                          {[1, 2, 3, 4, 5, 6, 7].map((r) => {
                            const cell = dayCells[r - 1];
                            if (r === 4) {
                              return (
                                <td key={`${name}-lunch`} style={{ background: 'repeating-linear-gradient(45deg, var(--divider), var(--divider) 6px, var(--secondary-lighter) 6px, var(--secondary-lighter) 12px)', padding: '12px 8px', fontSize: 11.5, textAlign: 'center', height: 64, color: 'var(--text-lighter)', fontStyle: 'italic', verticalAlign: 'middle' }}>
                                  Lunch break
                                </td>
                              );
                            }
                            if (cell === null || cell === 'empty' || cell === 'lunch') {
                              return <td key={`${name}-${r}`} style={{ background: 'var(--surface-soft)', padding: '12px 8px', height: 64 }} />;
                            }
                            return (
                              <td key={`${name}-${r}`} colSpan={cell.span} style={{ background: 'linear-gradient(135deg, #e8f4fc, #d0e8f5)', borderRadius: 6, margin: 2, padding: '12px 8px', fontSize: 11.5, textAlign: 'center', height: 64, verticalAlign: 'middle' }}>
      ...
      {showExport && (
        <ExportTimetableModal
          ...
          schedules={all}
          sourceLabel={activeTerm ? `Published — AY ${activeTerm.academicYear}` : 'Published Timetable'}
          ...
          {settingsTab === 'Profile' && (
            <div className="bg-base-100 backdrop-blur-xl" style={{ borderRadius: 'var(--radius-lg)', border: '1px solid var(--surface-border)', boxShadow: 'var(--shadow-sm)', overflow: 'hidden' }}>
              ...
              <div style={{ padding: '16px 22px' }}>
                ...
                <>
                <div style={{ marginBottom: 16 }}><label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--accent)', marginBottom: 6 }}>Full Name</label><input type="text" defaultValue={name} readOnly style={{ width: '100%', padding: '10px 14px', borderRadius: 'var(--radius-sm)', border: '1.5px solid var(--surface)', background: 'var(--surface-soft)', fontSize: 13, color: 'var(--text)', cursor: 'default' }} /></div>
                <div style={{ marginBottom: 16 }}><label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--accent)', marginBottom: 6 }}>Email</label><input type="email" defaultValue={me} readOnly style={{ width: '100%', padding: '10px 14px', borderRadius: 'var(--radius-sm)', border: '1.5px solid var(--surface)', background: 'var(--surface-soft)', fontSize: 13, color: 'var(--text)', cursor: 'default' }} /></div>
                <div style={{ marginBottom: 16 }}><label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--accent)', marginBottom: 6 }}>Phone Number</label><input type="tel" defaultValue={profile?.phone || ''} readOnly style={{ width: '100%', padding: '10px 14px', borderRadius: 'var(--radius-sm)', border: '1.5px solid var(--surface)', background: 'var(--surface-soft)', fontSize: 13, color: 'var(--text)', cursor: 'default' }} /></div>
                <div style={{ marginBottom: 16 }}><label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--accent)', marginBottom: 6 }}>Bio</label><textarea readOnly placeholder="No bio" value="" style={{ width: '100%', padding: '10px 14px', borderRadius: 'var(--radius-sm)', border: '1.5px solid var(--surface)', background: 'var(--surface-soft)', fontSize: 13, color: 'var(--text-lighter)', fontFamily: 'inherit', resize: 'none', minHeight: 80, cursor: 'default' }} /></div>
                </>
                )}
              </div>
            </div>
