...
export default function FeedSection() {
  ...
  const { posts, loading, loadingMore, hasMore, loadMore, hasError, refresh } = useFeedPosts({ hashtag: hashtag ?? null });
  ...
  useEffect(() => {
    ...
    if (tag == null) return;
    const t = setTimeout(() => setHashtag(tag.replace(/^#/, '')), 0);
    return () => clearTimeout(t);
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener('uniconnect-hashtag', handler);
    return () => window.removeEventListener('uniconnect-hashtag', handler);
  }, []);
  ...
  useEffect(() => {
    ...
    const fetchPost = async () => {
      try {
        ...
        const data = json.data as Post | null;
  ...
  const displayPosts = useMemo(() => {
    ...
    if (hashtag) {
      const target = normToken(hashtag);
      base = base.filter((p) => {
        const tags = Array.isArray(p.tags) ? (p.tags as { label?: string }[]) : [];
        if (tags.some((t) => normToken(t.label ?? '') === target)) return true;
        for (const m of (p.content ?? '').matchAll(/#[\w&]+/gi)) {
          if (normToken(m[0]) === target) return true;
        }
        return false;
      });
    }
