...
export default function SearchModal({ open, onClose }: SearchModalProps) {
  ...
  const countHashtag = (tag: string) => {
    const norm = tag.replace(/^#/, '').toLowerCase().replace(/[^a-z0-9]/gi, '');
    if (!norm) return;
  ...
  const filteredHashtags = Array.from(hashtagCounts.entries())
    ...
    .filter((h) => h.tag.includes(cleanQuery));
  const queryIsHashtag = cleanQuery.length > 0 && /^[\w&]+$/.test(cleanQuery);
  if (queryIsHashtag && !filteredHashtags.some((h) => h.tag === cleanQuery)) {
    filteredHashtags.unshift({ tag: cleanQuery, posts: 0 });
  }
  ...
  const openHashtag = useCallback((tag: string) => {
    ...
    if (typeof window !== 'undefined') {
      window.setTimeout(() => {
        window.dispatchEvent(new CustomEvent('uniconnect-hashtag', { detail: tag }));
      }, 120);
    }
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span style={{ fontSize: 11, color: 'var(--text-lighter)' }}>{new Date(p.created_at * 1000).toLocaleDateString()}</span>
                  </div>
