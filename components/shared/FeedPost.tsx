...
import { usePostVideoDownload } from '@/lib/usePostVideo';
...
import { usePostLikes, useComments, usePostShares, type Post, type Comment } from '@/lib/hooks';
import type { UniversityPerson } from './useUniversityPeople';
import { useUniversityPeople } from './useUniversityPeople';

const FEED_MEDIA_HEIGHT = 320;
...
function PostImage({ postId }: { postId: string }) {
  ...
  if (phase === 'downloading' || phase === 'retrying') {
    return <PostImageDownload height={FEED_MEDIA_HEIGHT} progress={progress} retrying={phase === 'retrying'} attemptsLeft={attemptsLeft} />;
  }
  if (phase === 'failed') {
    return (
      <div
        className="mt-3 w-full flex flex-col items-center justify-center gap-1"
        style={{ height: FEED_MEDIA_HEIGHT, borderRadius: 'var(--radius-md)', border: '1px dashed var(--surface-border)', background: 'var(--divider-soft)', color: 'var(--text-lighter)', fontSize: 12 }}
      >
        <ImageOff size={18} />
  ...
  return (
    <>
      ...
      >
        <img src={src} alt="" loading="lazy" className="w-full h-full" style={{ objectFit: 'contain' }} />
      </div>
      {showImage && (
        <ImageLightbox
          open={showImage}
...
function PostVideo({ videoUrl }: { videoUrl: string }) {
  ...
  return (
    ...
    >
      {phase === 'done' && src ? (
        <video src={src} controls playsInline className="w-full h-full" style={{ objectFit: 'contain' }} />
      ) : phase === 'failed' ? (
        <div
          className="w-full flex flex-col items-center justify-center gap-1"
          style={{ height: '100%', borderRadius: 'var(--radius-md)', border: '1px dashed var(--surface-border)', background: 'var(--divider-soft)', color: 'var(--text-lighter)', fontSize: 12 }}
        >
          <VideoOff size={18} />
          <span>Video unavailable</span>
        </div>
      ) : (
        <PostImageDownload height={FEED_MEDIA_HEIGHT} progress={progress} retrying={phase === 'retrying'} attemptsLeft={attemptsLeft} label="Downloading video…" />
      )}
    </div>
  );
}
...
export default function FeedPost({ post }: FeedPostProps) {
  ...
  const authorName = useMemo(() => {
    const notEmail = (v?: string | null) => !!v && !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(v);
    const match = people.find((p) => p.email.toLowerCase() === post.author_email?.toLowerCase());
    const resolved = notEmail(match?.name) ? match!.name : '';
    const stored = notEmail(post.author_name) ? post.author_name! : '';
    return resolved || stored || post.author_email?.split('@')[0] || 'User';
  ...
  const truncatedText = isLongText ? contentText.slice(0, 150).trimEnd() : contentText;

  const isLiked = optimisticLiked ?? liked;
  const likesCount = optimisticLikes ?? likes;

  useEffect(() => {
    if (optimisticLiked === null && optimisticLikes === null) return;
    const id = setTimeout(() => {
  ...
  const toggleLike = useCallback(async (e: React.MouseEvent) => {
    ...
    const next = !isLiked;
    setLikePulse((c) => c + 1);
    const base = likesCount ?? post.likes_count ?? 0;
    setOptimisticLiked(next);
    setOptimisticLikes(next ? base + 1 : Math.max(base - 1, 0));
    try {
      ...
      const serverLikes = typeof body?.likes === 'number' ? body.likes : (next ? base + 1 : Math.max(base - 1, 0));
      applyLikeState(next, serverLikes);
      setOptimisticLiked(null);
      setOptimisticLikes(null);
    } catch (err) {
      setOptimisticLiked(null);
  ...
  }, [me, isLiked, liked, likes, likesCount, post.id, post.likes_count, applyLikeState]);
...
<Link
            ...
            <span style={{ fontSize: 12, color: 'var(--text-lighter)', fontWeight: 500 }}>
              {post.author_role && `${post.author_role} \u2022 `}{timeAgo(post.created_at * 1000)}
            </span>
          {isEdited && (
        ...
        {editing ? (
          ...
          <div style={{ fontSize: 14, color: 'var(--text)', marginTop: 5, lineHeight: 1.6 }}>
            {isLongText ? (
              <span
                onClick={(e) => { e.stopPropagation(); setShowDetail(true); }}
                style={{ cursor: 'pointer' }}
              >
                {truncatedText}… <span style={{ color: 'var(--primary)', fontWeight: 600 }}>Read more</span>
              </span>
            ) : (
              post.content
            )}
        ...
        <div className="flex gap-[6px] mt-[10px] flex-wrap">
          ...
          {(Array.isArray(post.tags)
            ? post.tags
            : (() => { try { return typeof post.tags === 'string' ? JSON.parse(post.tags) : []; } catch { return []; } })()
          ).map((tag: unknown, i: number) => (
            <PostTag key={`${(tag as { label?: string }).label ?? ''}-${i}`} label={(tag as { label: string }).label} emoji={(tag as { emoji?: string }).emoji} />
          ))}
        </div>
        <div
        ...
        >
          <div className="flex items-center" style={{ gap: 2 }}>
            ...
            <button
              onClick={isOwner ? () => setShowLikers(true) : undefined}
              ...
              style={{ color: 'var(--text-light)', background: 'transparent', cursor: isOwner ? 'pointer' : 'default' }}
              onMouseEnter={(e) => { if (isOwner) { e.currentTarget.style.color = 'var(--primary)'; e.currentTarget.style.backgroundColor = 'rgba(40, 114, 161,0.12)'; } }}
              onMouseLeave={(e) => { if (isOwner) { e.currentTarget.style.color = 'var(--text-light)'; e.currentTarget.style.backgroundColor = 'transparent'; } }}
            >
              {likesCount ?? '—'}
            </button>
          </div>
      ...
      {showDetail && createPortal(
        <AnimatePresence>
          <motion.dialog
            ...
            onCancel={(e) => { e.preventDefault(); setShowDetail(false); }}
          >
            <motion.div
              ...
              className="w-[94vw] max-w-2xl bg-base-100 rounded-2xl overflow-hidden"
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 20px', borderBottom: '1px solid var(--surface)' }}>
                <Link
                  href={`/people/${encodeURIComponent(post.author_email)}`}
                  className="w-10 h-10 rounded-full bg-gradient-to-br from-primary to-secondary flex items-center justify-center text-white font-bold shrink-0 no-underline"
                  style={{ fontSize: 13 }}
                >
                  {post.author_initials}
                </Link>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <Link
                      href={`/people/${encodeURIComponent(post.author_email)}`}
                      className="no-underline hover:underline font-semibold text-sm"
                ...
                <button onClick={() => setShowDetail(false)} className="btn btn-ghost btn-circle btn-sm" title="Close" style={{ color: 'var(--text-light)' }}>
                  <X size={16} />
                </button>
              </div>
              <div className="overflow-y-auto" style={{ maxHeight: '60vh', padding: '18px 20px' }}>
                <div style={{ fontSize: 14.5, color: 'var(--text)', lineHeight: 1.7, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{post.content}</div>
                ...
                <div className="flex items-center gap-[18px] mt-5 pt-3" style={{ borderTop: '1px solid var(--divider)' }}>
                  <span className="flex items-center gap-1.5 text-xs font-semibold" style={{ color: isLiked ? 'var(--primary)' : 'var(--text-light)' }}>
                    <Heart size={14} fill={isLiked ? 'currentColor' : 'none'} /> {likesCount ?? 0}
                  </span>
                  <span className="flex items-center gap-1.5 text-xs font-semibold" style={{ color: 'var(--text-light)' }}>
                    <MessageCircle size={14} /> {comments?.length ?? post.comments_count ?? 0}
                  </span>
                  <span className="flex items-center gap-1.5 text-xs font-semibold" style={{ color: 'var(--text-light)' }}>
                    <Share2 size={14} /> {shares ?? 0}
                  </span>
                </div>
              </div>
            </motion.div>
