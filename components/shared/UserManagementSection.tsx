...
import {
  GraduationCap, Presentation, ShieldCheck, UserCog, Landmark, Plus, Search, Power, Mail, Pencil, Trash2, Upload, CheckSquare, CircleCheck,
} from 'lucide-react';
...
export type UserTarget = 'students' | 'lecturers' | 'student-affairs' | 'administrative-officers' | 'admins';
...
export default function UserManagementSection({ target }: { target: UserTarget }) {
  ...
  const availableUnits = useMemo(() => {
    ...
    if (target === 'student-affairs' || target === 'administrative-officers') {
  ...
  const allRows: Row[] = useMemo(() => {
    ...
    return out;
  }, [data?.users, staffByUser, studentsByUser, meta.roleName, staffPosition, target]);

  const semesterOptions = useMemo(
    () => Array.from(new Set(allRows.map((r) => r.semester).filter((s): s is number => s !== null))).sort((a, b) => a - b),
    [allRows]
  );
  const sectionOptions = useMemo(
    () => Array.from(new Set(allRows.map((r) => r.section).filter((s): s is string => !!s))).sort(),
    [allRows]
  );
  const majorOptions = useMemo(
    () => Array.from(new Set(allRows.map((r) => r.major).filter((m): m is string => !!m))).sort(),
    [allRows]
  );
  const unitOptions = useMemo(
    () => Array.from(new Set(allRows.map((r) => r.unit).filter((u): u is string => !!u))).sort(),
    [allRows]
  );

  const rows = useMemo(() => {
    const q = search.toLowerCase();
    const staffTarget = target !== 'students' && target !== 'admins';
    return allRows.filter((r) => {
      if (target === 'students') {
        if (semesterFilter && r.semester !== Number(semesterFilter)) return false;
        if (sectionFilter && r.section !== sectionFilter) return false;
        if (majorFilter && r.major !== majorFilter) return false;
      } else if (staffTarget && unitFilter && r.unit !== unitFilter) {
        return false;
      }
      return (
  ...
  }, [allRows, search, semesterFilter, sectionFilter, majorFilter, unitFilter, target]);
    ...
    <div>
      ...
      {target !== 'admins' && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18, flexWrap: 'wrap' }}>
          {target === 'students' ? (
            <>
              <select style={filterSelectStyle} value={semesterFilter} onChange={(e) => { setSemesterFilter(e.target.value); setPage(0); }}>
                <option value="">All semesters</option>
                {semesterOptions.map((s) => (
                  <option key={s} value={s}>Semester {s}</option>
                ))}
              </select>
              <select style={filterSelectStyle} value={sectionFilter} onChange={(e) => { setSectionFilter(e.target.value); setPage(0); }}>
                <option value="">All sections</option>
                {sectionOptions.map((s) => (
                  <option key={s} value={s}>{s}</option>
                ))}
              </select>
              <select style={filterSelectStyle} value={majorFilter} onChange={(e) => { setMajorFilter(e.target.value); setPage(0); }}>
            ...
            <select style={filterSelectStyle} value={unitFilter} onChange={(e) => { setUnitFilter(e.target.value); setPage(0); }}>
          ...
          {(semesterFilter || sectionFilter || majorFilter || unitFilter) && (
            <button
              onClick={() => { setSemesterFilter(''); setSectionFilter(''); setMajorFilter(''); setUnitFilter(''); setPage(0); }}
              style={{ padding: '9px 12px', fontSize: 13, fontWeight: 600, color: 'var(--primary)', background: 'rgba(40,114,161,0.1)', border: '1.5px solid rgba(40,114,161,0.3)', borderRadius: 'var(--radius-md)', cursor: 'pointer' }}
            >
              Clear filters
            </button>
          )}
...
function EditModal({ target, row, studentsByUser, staffByUser, majors, semesters, sections, terms, availableUnits, saving, onSave, onCancel }: EditModalProps) {
  ...
  return (
    <div style={overlayStyle}>
      <div className="bg-base-100 w-full max-w-2xl" style={{ borderRadius: 'var(--radius-lg)', border: '1px solid var(--surface-border)', boxShadow: 'var(--shadow-lg)', overflow: 'hidden', maxHeight: '92vh' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid var(--surface)' }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--accent)', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div className="grid grid-cols-2 gap-4 p-5 overflow-y-auto" style={{ maxHeight: 'calc(92vh - 120px)' }}>
          ...
          {isStaffTarget && (
            <>
              ...
              <div className="col-span-2" style={{ background: 'var(--divider)', borderRadius: 'var(--radius-md)', padding: 10 }}>
          ...
          <label className="col-span-2 flex items-center gap-2 text-sm cursor-pointer" style={{ color: 'var(--text-light)' }}>
