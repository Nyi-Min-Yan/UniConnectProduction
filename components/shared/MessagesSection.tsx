...
import { useUniversityPeople, useUniversityRaw, initialsOf } from './useUniversityPeople';
...
const emailDisplayName = (email: string): string => {
  const local = (email || '').split('@')[0] || email || '';
  const words = local.split(/[._-]+/).filter(Boolean);
  return words.length > 1 ? words.map((w) => w[0].toUpperCase() + w.slice(1)).join(' ') : local;
};
...
function parseAttachments(raw: unknown): MessageAttachment[] | undefined {
  ...
  return normalizeAttachments(arr);
}
...
function toChatMessage(m: ChatMessageRow): ChatMessage {
  ...
  return {
    ...
    editedAt: Number.isNaN(editedAt) ? null : (editedAt as number | null),
    isDeleted: (row.is_deleted as boolean | undefined) ?? false,
    is_read: row.is_read as boolean | undefined,
    attachments,
  };
}
...
function parseParticipantMeta(raw: unknown): { email: string; name: string; initials: string }[] {
  const normalize = (arr: unknown[]): { email: string; name: string; initials: string }[] =>
    arr.filter((x): x is { email: string; name: string; initials: string } =>
      !!x && typeof x === 'object' && typeof (x as { email?: unknown }).email === 'string'
    );
...
export default function MessagesSection() {
  ...
  const reloadConversations = useCallback(async () => {
    ...
    try {
      ...
      const { conversations: docs } = await res.json();
      const enriched: ConvInfo[] = ((docs as unknown[]) ?? []).map((row) => {
        const conv = row as Record<string, unknown>;
        const meta = parseParticipantMeta(conv.participant_meta);
        const participants = parseParticipantIds(conv.participant_ids);
        ...
        const other = meta.find((m) => m.email === otherEmail) || { email: otherEmail, name: emailDisplayName(otherEmail), initials: initialsOf(emailDisplayName(otherEmail)) };
        const groupIdx = meta.findIndex((m) => m.email === GROUP_META_EMAIL);
        const creatorEmail = isGroup
          ? (groupIdx >= 0 ? (meta[groupIdx + 1]?.email ?? '') : '')
          : '';
        const unreadMap = parseNumMap(conv.unread_map);
        const hiddenMap = parseNumMap(conv.hidden_map);
        return {
          id: (conv.id as string) ?? '',
          status: (conv.status as ConvStatus) || 'active',
          requestedBy: (conv.requested_by as string) ?? '',
          blockedBy: (conv.blocked_by as string) ?? '',
          lastMessageAt: (conv.last_message_at as number) || 0,
          preview: (conv.preview as string) ?? '',
          unread: unreadMap[me] ?? 0,
          hiddenAt: hiddenMap[me] as number | undefined,
          isGroup,
  ...
  useEffect(() => {
    if (!socket || !me) return;
    const onConvEvent = () => { void reloadConversations(); };
    const onMsg = (d: Record<string, unknown>) => {
      if (d?.sender_email === me) return;
      void reloadConversations();
    };
    socket.on(WS_EVENTS.CONVERSATION_CREATED, onConvEvent);
    socket.on(WS_EVENTS.CONVERSATION_UPDATED, onConvEvent);
    socket.on(WS_EVENTS.CONVERSATION_BLOCKED, onConvEvent);
    socket.on(WS_EVENTS.CONVERSATION_UNBLOCKED, onConvEvent);
    socket.on(WS_EVENTS.MESSAGE_CREATED, onMsg);
    socket.on(WS_EVENTS.MESSAGE_READ, onConvEvent);
    return () => {
      socket.off(WS_EVENTS.CONVERSATION_CREATED, onConvEvent);
      socket.off(WS_EVENTS.CONVERSATION_UPDATED, onConvEvent);
      socket.off(WS_EVENTS.CONVERSATION_BLOCKED, onConvEvent);
      socket.off(WS_EVENTS.CONVERSATION_UNBLOCKED, onConvEvent);
      socket.off(WS_EVENTS.MESSAGE_CREATED, onMsg);
      socket.off(WS_EVENTS.MESSAGE_READ, onConvEvent);
    };
  }, [socket, me, reloadConversations]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`/api/conversations/${selectedId}/messages?limit=${MESSAGE_PAGE}`);
        if (!res.ok) throw new Error('Failed to load messages');
        const { messages: docs } = await res.json();
        if (cancelled) return;
        setMessages(((docs as unknown as ChatMessageRow[]) ?? []).map(toChatMessage));
        setHasMoreMsgs((docs?.length ?? 0) === MESSAGE_PAGE);
      } catch {
    ...
    fetch(`/api/conversations/${selectedId}/messages/reads`)
      ...
      .then((d) => {
        if (cancelled) return;
        const rows = ((d?.reads ?? []) as { message_id: string; reader_email: string; read_at: number }[]);
  ...
  useEffect(() => {
    ...
    const onCreated = (data: unknown) => {
      const d = data as { conversation_id?: string; id?: string; sender_email?: string };
      if (d.conversation_id !== selectedId) return;
      const msgId = d.id ?? '';
      const m = toChatMessage((data ?? {}) as Record<string, unknown>);
      setMessages((prev) => {
        if (!prev) return prev;
        const existing = prev.find((x) => x.id === msgId);
        if (existing) return prev.map((x) => (x.id === msgId ? { ...x, ...m, pending: false } : x));
        if (d.sender_email !== me) markMessagesRead();
        return [...prev, m];
      });
    };
    const onUpdated = (data: unknown) => {
      const d = data as { conversation_id?: string; id?: string };
      if (d.conversation_id !== selectedId) return;
      const msgId = d.id ?? '';
      const m = toChatMessage((data ?? {}) as Record<string, unknown>);
      setMessages((prev) => prev?.map((x) => (x.id === msgId ? { ...x, ...m, attachments: m.attachments ?? x.attachments } : x)) ?? prev);
    };
    const onDeleted = (data: unknown) => {
      const d = data as { conversation_id?: string; id?: string };
      if (d.conversation_id !== selectedId) return;
      setMessages((prev) => prev?.filter((m) => m.id !== d.id) ?? prev);
    };
    const onRead = (data: unknown) => {
      const d = data as { conversation_id?: string; message_ids?: string[]; reader_email?: string };
      if (d.conversation_id !== selectedId) return;
      const ids = new Set(d.message_ids ?? []);
      if (ids.size === 0) return;
      const reader = d.reader_email;
      const readAt = Math.floor(Date.now() / 1000);
      setReadMap((prev) => {
        const next: Record<string, { email: string; readAt: number }[]> = { ...prev };
        ids.forEach((mid) => {
          const list = next[mid] ?? [];
          if (reader && !list.some((r) => r.email === reader)) {
            next[mid] = [...list, { email: reader, readAt }];
          }
        });
        return next;
      });
      setMessages((prev) => prev?.map((m) => (ids.has(m.id) ? { ...m, is_read: true } : m)) ?? prev);
    };
    socket.on(WS_EVENTS.MESSAGE_CREATED, onCreated);
    socket.on(WS_EVENTS.MESSAGE_UPDATED, onUpdated);
    socket.on(WS_EVENTS.MESSAGE_DELETED, onDeleted);
    socket.on(WS_EVENTS.MESSAGE_READ, onRead);
    return () => {
      socket.off(WS_EVENTS.MESSAGE_CREATED, onCreated);
      socket.off(WS_EVENTS.MESSAGE_UPDATED, onUpdated);
      socket.off(WS_EVENTS.MESSAGE_DELETED, onDeleted);
      socket.off(WS_EVENTS.MESSAGE_READ, onRead);
    };
  }, [socket, selectedId, me, markMessagesRead]);
  ...
  useEffect(() => {
    ...
    if (!socket || !selectedId || !me) return;
    socket.emit('join', `conversation:${selectedId}`);
    const onTypingStart = (data: { conversationId: string; userEmail: string }) => {
      if (!data || data.conversationId !== selectedId || data.userEmail === me) return;
      setIsTyping(true);
      if (typingTimerRef.current) clearTimeout(typingTimerRef.current);
      typingTimerRef.current = setTimeout(() => setIsTyping(false), 2500);
    };
    const onTypingStop = (data: { conversationId: string; userEmail: string }) => {
      if (!data || data.conversationId !== selectedId || data.userEmail === me) return;
      setIsTyping(false);
    };
    socket.on(WS_EVENTS.TYPING_START, onTypingStart);
    socket.on(WS_EVENTS.TYPING_STOP, onTypingStop);
    return () => {
      socket.emit('leave', `conversation:${selectedId}`);
      socket.off(WS_EVENTS.TYPING_START, onTypingStart);
      socket.off(WS_EVENTS.TYPING_STOP, onTypingStop);
      if (typingTimerRef.current) {
        clearTimeout(typingTimerRef.current);
        typingTimerRef.current = null;
      }
    };
  }, [socket, selectedId, me]);
  ...
  useEffect(() => {
    ...
    const onCreated = (payload: unknown) => {
      const row = (payload ?? {}) as { conversation_id?: string; sender_email?: string };
      if (!row?.sender_email || row.sender_email === me) return;
    ...
    socket.on(WS_EVENTS.MESSAGE_CREATED, onCreated);
    return () => { socket.off(WS_EVENTS.MESSAGE_CREATED, onCreated); };
  }, [socket, me]);
  ...
  const loadOlder = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/conversations/${selectedId}/messages?limit=${MESSAGE_PAGE}&before=${oldest.created_at}`);
      if (!res.ok) throw new Error('Failed to load messages');
      const { messages: docs } = await res.json();
      const batch: ChatMessage[] = ((docs as unknown as ChatMessageRow[]) ?? []).map(toChatMessage);
      setMessages((prev) => {
  ...
  }, [selectedId, loadingOlder, hasMoreMsgs, messages]);
  ...
  const saveEditMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) => prev?.map((m) => (m.id === msgId ? { ...m, content, editedAt: Date.now() } : m)) ?? prev);
      cancelEditMessage();
    } catch (e) {
  ...
  const mentionCandidates = (() => {
    ...
    for (const m of selected.members) {
      ...
      seen.add(m.email);
      out.push({ key: m.email, token: m.name, name: `@${m.name}`, sub: people.find((p) => p.email === m.email)?.role ?? '' });
    }
  ...
  }, [selectedId, me, reloadConversations]);
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="flex items-center gap-1.5 shrink-0">
                        ...
                        <span style={{ fontSize: 11, color: 'var(--text-lighter)' }}>{timeLabel(conv.lastMessageAt * 1000)}</span>
                      </span>
                    </div>
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-1.5" style={{ fontSize: 11.5, color: 'var(--text-lighter)' }}>
                  {!selected.isGroup && selected.status === 'active' && isTyping ? (
                    ...
                    `Last seen ${lastSeenLabel(displayLastSeen * 1000)}`
                  ) : selected.isGroup ? (
                  ...
                  >
                    {showSender && (
                      <div className="w-7 h-7 rounded-full bg-gradient-to-br from-secondary to-primary flex items-center justify-center text-white font-bold shrink-0 mr-2 mt-1" style={{ fontSize: 9 }}>
                        {initialsOf(m.sender_name || emailDisplayName(m.sender_email))}
                      </div>
                    )}
                    <div className="relative max-w-[75%]">
                      ...
                      >
                        {m.isDeleted ? (
                          ...
                          <>
                            {showSender && (
                              <div className="text-[10.5px] font-semibold mb-0.5" style={{ color: 'var(--primary)' }}>{m.sender_name || emailDisplayName(m.sender_email)}</div>
                            )}
                            {m.content && (
                        ...
                        <div className="text-[10px] mt-1 flex items-center gap-1.5" style={{ opacity: 0.7 }}>
                          <span>{timeLabel(m.created_at * 1000)}</span>
                                            ...
                                            return (
                                              <div key={r.email} className="flex items-center gap-2 mb-1.5 last:mb-0">
                                                <div className="w-6 h-6 rounded-full bg-gradient-to-br from-secondary to-primary flex items-center justify-center text-white font-bold shrink-0" style={{ fontSize: 8 }}>
                                                  {member?.initials ?? initialsOf(emailDisplayName(r.email))}
                                                </div>
                                                <div className="min-w-0 flex-1">
                                                  <div className="truncate" style={{ fontSize: 11.5, fontWeight: 600, color: 'var(--text)' }}>
                                                    {member?.name ?? emailDisplayName(r.email)}
                                                  </div>
                      ...
                      <span className="text-xs font-semibold" style={{ color: mentionMemberOnline ? '#4ade80' : '#94a3b8' }}>
                        {mentionMemberOnline
                          ...
                          : mentionMemberLastSeen
                            ? `Last seen ${lastSeenLabel(mentionMemberLastSeen * 1000)}`
                            : 'Offline'}
                      </span>
                    </div>
                  </div>
