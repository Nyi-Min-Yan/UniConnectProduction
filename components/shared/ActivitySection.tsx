...
import { Heart, MessageCircle, Share2, Plus, X, Send, Play, Trash2, Music2, Clapperboard, Loader2, Download, Volume2, VolumeX } from 'lucide-react';
import { toast } from 'sonner';
import { useActivities, useActivityLikes, useActivityComments, useActivityShares } from '@/lib/hooks';
import type { Activity, ActivityComment } from '@/lib/hooks';
import { useSession } from './session';
import ShareModal from './ShareModal';
import MediaCarousel from './MediaCarousel';
import type { UniversityPerson } from './useUniversityPeople';

function mediaUrlsOf(activity: Activity): string[] {
  const raw: unknown = activity.media_urls;
...
function ActivityMedia({ activity, videoRef, muted, onOpenDetails }: { activity: Activity; videoRef?: (node: HTMLVideoElement | null) => void; muted: boolean; onOpenDetails?: () => void }) {
  ...
  if (activity.kind === 'photo') {
    const urls = mediaUrlsOf(activity);
    if (urls.length > 1) {
      return (
        <div className="w-full h-full">
          <MediaCarousel urls={urls} alt={activity.caption ?? ''} maxHeight={520} />
        </div>
      );
    }
    if (activity.media_url) {
      return <img src={activity.media_url} alt={activity.caption ?? ''} loading="lazy" decoding="async" className="w-full h-full object-cover" />;
    }
  }
  return (
    <div
      className="w-full h-full flex flex-col items-center justify-center px-8 text-center cursor-pointer"
    ...
    >
      ...
      >
        {activity.caption ? truncateWords(activity.caption, 100) : ''}
      </div>
      {activity.caption && (
        <span className="mt-3 inline-flex items-center gap-1 px-3 py-1 rounded-full text-white/90" style={{ fontSize: 12, background: 'rgba(255,255,255,0.14)', backdropFilter: 'blur(6px)' }}>
          Read more
        </span>
      )}
    </div>
...
function ActivitySlide({ activity, playing, onTogglePlay, onOpenComments, canManage, onDelete, videoRef }: ActivitySlideProps) {
  ...
  const { liked, likes } = useActivityLikes(activity.id, me);
  const { comments } = useActivityComments(activity.id);
  const { shares } = useActivityShares(activity.id);
  ...
  return (
    <div className="relative w-full h-full overflow-hidden select-none" style={{ background: '#000' }}>
      ...
      >
        <ActivityMedia activity={activity} videoRef={videoRef} muted={muted} onOpenDetails={() => setShowDetails(true)} />
      </button>

      {activity.kind === 'video' && activity.media_url && (
        <div className="absolute top-16 right-3 z-20 flex flex-row items-center gap-2">
          <button
            type="button"
            onClick={toggleMute}
      ...
      <div className="absolute bottom-0 left-0 right-16 lg:right-20 px-4 pb-6 z-10 text-left">
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <span className="text-white/80" style={{ fontSize: 11, textShadow: '0 1px 6px rgba(0,0,0,0.6)' }}>{timeAgo(activity.created_at * 1000)}</span>
        </div>
        {activity.caption && activity.kind !== 'text' && (
          <button
            ...
            title="View full description"
            className="text-white mb-1.5 text-left cursor-pointer border-none p-0 m-0"
            style={{ fontSize: 13.5, lineHeight: 1.45, textShadow: '0 1px 6px rgba(0,0,0,0.6)', maxWidth: 460 }}
          >
            {truncateWords(activity.caption, 100)}
          </button>
        )}
      ...
      <ActivityDetailsModal
        ...
        activity={activity}
        likes={likesCount ?? 0}
        comments={comments?.length ?? activity.comments_count ?? 0}
        shares={shares ?? activity.shares_count ?? 0}
      />
    </div>
  );
}

function ActivityDetailsModal({ activity, likes, comments, shares, open, onClose }: {
  activity: Activity;
  likes: number;
  comments: number;
  shares: number;
  ...
  useEffect(() => {
    const el = dialogRef.current;
    if (!el) return;
    const handler = () => { onClose(); document.body.style.overflow = ''; };
    el.addEventListener('close', handler);
    return () => el.removeEventListener('close', handler);
  }, [onClose]);

  const dateLabel = new Date(activity.created_at * 1000).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' });
    ...
    >
      <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--surface)' }}>
        <div className="flex items-center justify-between mb-4">
          <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--accent)', margin: 0 }}>Activity details</h3>
        ...
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-full flex items-center justify-center text-white font-bold shrink-0" style={{ fontSize: 13, background: 'linear-gradient(135deg, var(--secondary-light), var(--secondary))' }}>
            {activity.author_initials}
          </div>
          <div className="flex-1 min-w-0">
            <div style={{ fontSize: 14.5, fontWeight: 700, color: 'var(--accent)' }}>{activity.author_name}</div>
            <div style={{ fontSize: 12, color: 'var(--text-lighter)', marginTop: 1 }}>
              {activity.author_role} • {activity.author_email}
            </div>
          </div>
          <div className="text-right shrink-0" style={{ fontSize: 11.5, color: 'var(--text-lighter)' }}>
            Posted {dateLabel}
          </div>
        </div>
      </div>

      <div className="overflow-y-auto" style={{ padding: '20px 24px', maxHeight: 320 }}>
        <div style={{ fontSize: 12, fontWeight: 600, letterSpacing: '0.4px', textTransform: 'uppercase', color: 'var(--text-lighter)', marginBottom: 8 }}>Description</div>
        <div style={{ fontSize: 14, lineHeight: 1.6, color: 'var(--text)', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
          {activity.caption || 'No description'}
        </div>
      </div>

      <div className="flex items-center gap-6 px-6 py-4" style={{ borderTop: '1px solid var(--surface)' }}>
        <div className="flex items-center gap-2" style={{ color: 'var(--text)' }}>
          <Heart size={18} fill="#ff2d55" color="#ff2d55" />
          <span style={{ fontSize: 14, fontWeight: 700 }}>{likes}</span>
        </div>
        <div className="flex items-center gap-2" style={{ color: 'var(--text)' }}>
          <MessageCircle size={18} fill="#fff" color="#fff" className="opacity-90" />
          <span style={{ fontSize: 14, fontWeight: 700 }}>{comments}</span>
        </div>
        <div className="flex items-center gap-2" style={{ color: 'var(--text)' }}>
          <Share2 size={18} fill="#fff" color="#fff" className="opacity-90" />
          <span style={{ fontSize: 14, fontWeight: 700 }}>{shares}</span>
        </div>
      </div>
    </dialog>
  );
...
function CommentsPanel({ activity, onClose }: { activity: Activity; onClose: () => void }) {
  ...
  const { comments, loading } = useActivityComments(activity.id);
    ...
    <div className="flex items-start gap-2.5 rounded-xl px-3 py-2.5" style={{ background: 'var(--glass-bg)', border: '1px solid var(--glass-border)' }}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          <span style={{ fontSize: 11, color: 'var(--text-lighter)', whiteSpace: 'nowrap' }}>{timeAgo(comment.created_at * 1000)}</span>
          <button
...
function UploadModal({ onClose, onPosted }: UploadModalProps) {
  ...
  const submit = async () => {
    if (uploading) return;
    if (kind !== 'text' && kind === 'video' && !file) { toast.error('Choose a video to upload'); return; }
    if (kind === 'photo' && photos.length === 0) { toast.error('Choose at least one photo to upload'); return; }
    ...
    try {
      ...
      if (kind === 'video' && file) formData.append('file', file);
      if (kind === 'photo') photos.forEach((p) => formData.append('files', p.file));
          ...
          {kind !== 'text' && (
            <div className="mb-4">
              <input
                ...
                className="hidden"
                onChange={(e) => {
                  if (kind === 'photo') Array.from(e.target.files ?? []).forEach((f) => acceptFile(f));
                  else acceptFile(e.target.files?.[0] ?? null);
                  e.target.value = '';
                }}
              />
              {kind === 'photo' && photos.length > 0 ? (
                <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${Math.min(photos.length, 3)}, minmax(0, 1fr))` }}>
                  {photos.map((p, i) => (
                    <div key={i} className="relative rounded-xl overflow-hidden" style={{ aspectRatio: '1 / 1', border: '1px solid var(--surface-border)' }}>
                      <img src={p.preview} alt={`Photo ${i + 1}`} className="w-full h-full object-cover" />
                      <button
                        type="button"
                        onClick={() => removePhoto(i)}
                        className="btn btn-ghost btn-circle btn-xs absolute top-1 right-1 bg-base-100"
                ...
                >
                  <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--accent)' }}>
                    {kind === 'video' ? '📹 Choose a video' : '🖼️ Choose photos'}
                  </span>
                  <span style={{ fontSize: 11.5, color: 'var(--text-lighter)' }}>
                    {kind === 'video' ? 'MP4 / WebM up to 100MB' : 'PNG, JPG or WebP up to 10MB · up to 6 photos'}
                  </span>
                </button>
              )}
            </div>
...
export default function ActivitySection() {
  ...
  const { activities, loading, hasError, refresh, loadMore, hasMore, loadingMore } = useActivities();
  ...
  return (
    <div className="relative h-full">
      <div
        ref={containerRef}
        className="h-[calc(100dvh-6rem)] lg:h-[calc(100dvh-7.5rem)] max-w-[520px] mx-auto overflow-y-auto scrollbar-hide snap-y snap-mandatory scroll-smooth"
