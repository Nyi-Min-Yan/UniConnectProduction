...
function parseParticipantIds(val: unknown): string[] {
...
function parseParticipantMeta(val: unknown): Array<{ email: string; name?: string; initials?: string }> {
...
export default function BlockedSection({ bare = false }: { bare?: boolean }) {
  ...
  useEffect(() => {
    if (!me) return;
    const enrichedFromRows = (convs: any[]) => {
      const enriched: BlockedConv[] = (convs ?? [])
        .filter((conv: any) => {
          try {
            const ids = parseParticipantIds(conv.participant_ids);
        ...
        .map((conv: any) => {
          ...
          return {
            ...
            createdAt: new Date((conv.created_at || 0) * 1000).getTime(),
    ...
    const load = async () => {
      try {
        const res = await fetch('/api/conversations?status=blocked');
        if (!res.ok) throw new Error('Failed to load blocked conversations');
        const { conversations } = await res.json();
        enrichedFromRows(conversations ?? []);
      } catch (error) {
        console.error(error);
        setBlocked([]);
      }
  ...
  useEffect(() => {
    ...
    const onBlocked = (data: { conversationId: string; blockedBy: string }) => {
      setBlocked(prev => {
        ...
        if (exists) {
          return prev.map(c => c.id === data.conversationId ? { ...c, status: 'blocked' as ConvStatus, blockedBy: data.blockedBy } : c);
        }
        return prev;
      });
    };
