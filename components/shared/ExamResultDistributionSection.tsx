...
import {
  ...
  Trash2, History, Layers, Sparkles, Search, RotateCcw,
} from 'lucide-react';
...
const ROLL_RE = /\((UCSTGO-\d+|USTGO-\d+)\)\.pdf$/i;
...
const apiListDocuments = async (collection: string, options: Record<string, unknown> = {}) => {
  try {
    if (collection === 'exam_results') {
      const params = new URLSearchParams();
      if (options.equal) {
        const [field, value] = options.equal as [string, string | string[]];
        if (field === 'batchId' || field === 'batch_id') params.set('batch_id', value as string);
      }
      const res = await fetch(`/api/exam-results?${params.toString()}`);
      if (!res.ok) return { documents: [], total: 0 };
      const data = await res.json();
      const docs = (data.results ?? []) as unknown[];
      return { documents: docs, total: docs.length };
    }
    if (collection === 'exam_result_batches') {
      const res = await fetch('/api/exam-results/batches');
      if (!res.ok) return { documents: [], total: 0 };
      const data = await res.json();
      const docs = (data.batches ?? data.results ?? []) as unknown[];
      return { documents: docs, total: docs.length };
    }
  } catch {
...
export default function ExamResultDistributionSection() {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const { documents } = await apiListDocuments('exam_results');
      if (!cancelled) setSentCount(documents.length);
    })();
    return () => { cancelled = true; };
  }, []);

  const loadBatches = useCallback(async () => {
    const { documents } = await apiListDocuments('exam_result_batches');
    if (documents) {
      setBatches((documents as unknown as ExamBatchRow[]).map((b) => ({ ...b, created_at: Number(b.created_at) * 1000 })));
    }
  }, []);
  ...
  const loadHistory = useCallback(async () => {
    ...
    if (batchFilter) opts.equal = ['batch_id', batchFilter];
    let results: HistoryRow[] = [];
    if (historyExamType !== 'ALL') {
      const { documents: typeBatches } = await apiListDocuments('exam_result_batches');
      const typeIds = (typeBatches as unknown as ExamBatchRow[]).filter((b) => b.exam_type === historyExamType).map((b) => b.id);
      if (typeIds.length === 0) {
        setHistory([]);
        setHistoryTotal(0);
        return;
      }
      const { documents } = await apiListDocuments('exam_results', opts);
      results = (documents as unknown as HistoryRow[]).filter((r) => r.batch_id && typeIds.includes(r.batch_id));
    } else {
      const { documents } = await apiListDocuments('exam_results', opts);
      results = documents as unknown as HistoryRow[];
    }
    if (historySemester !== 'ALL') results = results.filter((r) => r.semester === historySemester);
    if (historySearch.trim()) {
      const q = historySearch.trim().toLowerCase();
      results = results.filter((r) => r.roll_number.toLowerCase().includes(q) || (r.student_name ?? '').toLowerCase().includes(q));
    }
    results = results
      .map((r) => ({ ...r, created_at: Number(r.created_at) * 1000 }))
      .sort((a, b) => b.created_at - a.created_at);
    const from = (historyPage - 1) * historyPageSize;
    setHistory(results.slice(from, from + historyPageSize));
    setHistoryTotal(results.length);
  }, [historyPage, historyPageSize, historyBatch, historySearch, historySemester, historyExamType]);
  ...
  useEffect(() => {
      ...
      if (d.id && d.status) {
        setBatches((prev) => {
          ...
          return prev.map((batch) =>
            batch.id === d.id ? { ...batch, status: d.status } : batch
          );
        });
        if (historyBatchRef.current === d.id || historyBatchRef.current === 'all') {
          void loadHistoryRef.current();
        }
      }
    };
    const onBatchDeleted = (data: unknown) => {
      const d = data as { id: string };
      if (d.id) {
        setBatches((prev) => (prev ?? []).filter((b) => b.id !== d.id));
        if (historyBatchRef.current === d.id || historyBatchRef.current === 'all') {
          void loadHistoryRef.current();
        }
      }
    ...
    socket.on(WS_EVENTS.EXAM_RESULT_BATCH_DELETED, onBatchDeleted);
    return () => {
      socket.off(WS_EVENTS.EXAM_RESULT_BATCH_UPDATED, onBatchUpdated);
      socket.off(WS_EVENTS.EXAM_RESULT_BATCH_DELETED, onBatchDeleted);
    };
  }, [socket]);
  ...
  const studentByRoll = useMemo(() => {
    ...
    const togglePrefix = (r: string) =>
      r.startsWith('UCSTGO') ? `USTGO${r.slice(6)}` : r.startsWith('USTGO') ? `UCSTGO${r.slice(5)}` : r;
    for (const s of students) {
      const roll = s.rollNo.toUpperCase();
      map.set(roll, s);
      map.set(togglePrefix(roll), s);
    }
    return map;
  }, [students]);
  ...
  const runBatchAction = async (batch: ExamBatchRow, action: 'publish') => {
    try {
