...
import { useEvents, useEventRegistrations } from '@/lib/hooks';
...
export default function EventsSection() {
  ...
  const { events, loading, hasError, refresh } = useEvents(role);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (selectedImage) {
        ...
        uploadForm.append('category', 'events');
        const uploadRes = await fetch('/api/files', { method: 'POST', body: uploadForm });
        const uploadData = await uploadRes.json().catch(() => ({}));
        if (!uploadRes.ok) throw new Error(uploadData.error || 'Could not upload cover image');
      ...
      formData.append('eventDate', String(dateMs));
      formData.append('maxAttendees', form.maxAttendees.trim() ? form.maxAttendees : '');
