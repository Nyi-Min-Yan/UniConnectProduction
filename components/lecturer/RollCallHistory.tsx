...
import { History, Users, Layers, AlertTriangle, Trash2, Download } from 'lucide-react';
import { toast } from 'sonner';
import { ExportAttendanceModal } from '@/components/shared/ExportModals';
import {
...
export function RollCallHistorySection({ seedScheduleId, seedMonth }: {
  ...
  const exportBounds = useMemo(() => monthBounds(month), [month]);
  ...
  const exportDownloadUrl = effCourse
    ? `/api/export/attendance?courseCode=${encodeURIComponent(effCourse)}&semesterNo=${parseInt(effSem) || 0}&sectionName=${encodeURIComponent(effSection)}&fromDate=${exportBounds.from}&toDate=${exportBounds.to}`
    : '';
  ...
  const pagedStudents = (data?.students ?? []).slice((histPage - 1) * 10, histPage * 10);
    ...
    <div>
      <div className="flex items-center justify-between gap-3 flex-wrap mb-4">
        ...
        <button
          ...
          style={{ background: 'linear-gradient(#16a34a, #12955f)', color: '#fff', borderRadius: 'var(--radius-sm)', padding: '9px 14px', fontSize: 12.5, fontWeight: 600, border: 'none', display: 'flex', alignItems: 'center', gap: 6, opacity: data && data.students.length > 0 ? 1 : 0.5 }}>
      ...
      <div className="flex items-end gap-3 flex-wrap mb-4">
        <label style={{ fontSize: 12, color: 'var(--text-light)', display: 'grid', gap: 4 }}>
          ...
          {!schedules ? (
            ...
            <select value={selCourse} onChange={(e) => { setSelCourse(e.target.value); setSelSection(''); }}
              ...
              {schedules.length === 0 && <option value="">No assigned courses available</option>}
        ...
        <label style={{ fontSize: 12, color: 'var(--text-light)', display: 'grid', gap: 4 }}>
          Semester
          {!schedules ? (
            <div className="skeleton h-9 w-32" style={{ borderRadius: 10 }} />
          ) : (
        ...
        <label style={{ fontSize: 12, color: 'var(--text-light)', display: 'grid', gap: 4 }}>
          Section
          {!schedules ? (
            <div className="skeleton h-9 w-28" style={{ borderRadius: 10 }} />
          ) : (
        ...
        <div>
          ...
          <div className="overflow-x-auto" style={{ border: '1px solid var(--surface-border)', borderRadius: 'var(--radius-lg)' }}>
            <table className="table w-full">
              <thead>
                <tr>{[...Array(5)].map((_, i) => <th key={i}><div className="skeleton h-3.5 w-16" /></th>)}</tr>
              </thead>
              <tbody>
                {[...Array(5)].map((_, r) => (
                  <tr key={r}>
                    {[...Array(5)].map((_, c) => <td key={c}><div className="skeleton h-3.5 w-20" /></td>)}
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <>
          ...
          {data.students.length > 10 && (
            <div className="join mt-3">
              <button className="join-item btn btn-sm cursor-pointer" disabled={histPage <= 1}
                onClick={() => setHistPage((p) => Math.max(1, p - 1))}>« Prev</button>
              <button className="join-item btn btn-sm no-animation" disabled>
                Page {histPage} of {Math.max(1, Math.ceil(data.students.length / 10))}
              </button>
              <button className="join-item btn btn-sm cursor-pointer"
                disabled={histPage >= Math.ceil(data.students.length / 10)}
