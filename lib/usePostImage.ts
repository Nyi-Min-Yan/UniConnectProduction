...
// Post images are inline base64 data URLs in posts.image. The single-post API
// returns the full row, so this streams the response body, deriving percent
// from Content-Length. The transfer can take a while on a slow connection, so
// instead of giving up and showing "Image unavailable", it retries with backoff
// until the image is viewable (or genuinely fails after MAX_ATTEMPTS).
...
async function downloadPostImage(
  ...
  if (!res.body) {
    ...
    return (data as { data?: { image?: string | null } }).data?.image ?? null;
  }
  ...
  const data = JSON.parse(text) as { data?: { image?: string | null } };
